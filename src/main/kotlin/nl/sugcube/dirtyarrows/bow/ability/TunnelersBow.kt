 ...
 * @author SugarCaney
 */
open class TunnelersBow(plugin: DirtyArrows) : BowAbility(
        plugin = plugin,
        type = DefaultBow.TUNNELER,
    ...
    override fun land(arrow: Arrow, player: Player, event: ProjectileHitEvent) {
        ...
        if (dx > dy && dx > dz) {
            hitBlock.location.breakSlicePerpendicularToXAxis(mineTool)
        }
        else if (dy > dz) {
            hitBlock.location.breakSlicePerpendicularToYAxis(mineTool)
        }
        else hitBlock.location.breakSlicePerpendicularToZAxis(mineTool)
     ...
     */
    private fun Location.breakSlicePerpendicularToXAxis(withTool: ItemStack) {
        for (y in yRange()) {
            for (z in zRange()) {
                world.getBlockAt(blockX, y, z).breakBlock(withTool)
            }
        }
    }
     ...
     *          The tool to use to break the blocks.
     */
    private fun Location.breakSlicePerpendicularToYAxis(withTool: ItemStack) {
        for (x in xRange()) {
            for (z in zRange()) {
                world.getBlockAt(x, blockY, z).breakBlock(withTool)
            }
        }
    }
     ...
     *          The tool to use to break the blocks.
     */
    private fun Location.breakSlicePerpendicularToZAxis(withTool: ItemStack) {
        for (x in xRange()) {
            for (y in yRange()) {
                world.getBlockAt(x, y, blockZ).breakBlock(withTool)
            }
        }
    }
